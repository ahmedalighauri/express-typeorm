...
describe('events', () => {
	...
	it(`GET /events/warmupevents`, async () => {
		const response = await request(Server.getApp().app).get('/events/warmupevents').expect(200)
	...
	it('GET /events/events', async () => {
		...
		const response = await request(Server.getApp().app).get('/events/events').expect(200)
	...
	it('GET /events/futureevents', async () => {
		const date2 = format(addYears(new Date(), 1), 'yyyy')
		const response = await request(Server.getApp().app).get('/events/futureevents').expect(200)
